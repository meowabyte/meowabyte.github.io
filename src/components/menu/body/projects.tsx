...
function Project({ repo: { name, description, stars, language } }: { repo: RepoData }) {
    ...
    return <div onClick={visitRepo} className="flex flex-col gap-1 min-w-52 border-2 border-foreground p-2 text-xs select-none cursor-pointer duration-100 hover:scale-105 active:scale-95">
        <div className="flex flex-row justify-between">
            <span className="font-bold"><HardDrive size="1.3em" className="inline" /> {name}</span>
            <Link2 size="1.3em" />
        </div>
