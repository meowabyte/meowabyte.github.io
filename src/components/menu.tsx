...
type MenuContextValue = {
    showModal: Dispatch<StateUpdater<ModalBodyType>>,
    readonly isModalSet: boolean
}
...
export function MenuProvider({ children }: { children: VNode }) {
    ...
    const contextValue = useMemo<MenuContextValue>(() => ({
        showModal: setModal,
        get isModalSet() { return CurrentModal !== null }
    }), [CurrentModal])
...
export default function Menu({ children, className }: { children: VNode<typeof MenuButton>[], className?: string }) {
    return <div class={cn(className, "flex flex-row gap-5 h-32 items-center")}>
        {children}
    </div>
}