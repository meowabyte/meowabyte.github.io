...
class SceneManager {
    ...
    private async init() {
        await Promise.all([this.initCamera(), this.initText()]);
...
type Props = { height: number; width: number; className?: string };
...
export default function SillyRenderer({ height, width, className }: Props) {
    ...
    return (
        <div class={cn(className, "group")}>
            ...
            />
            {shouldShowTip && (
                <ArrowLeftRight class="absolute left-1/2 bottom-5 scale-3d scale-y-75 transition-opacity duration-100 opacity-0 group-hover:!opacity-75" />
            )}
        </div>
    );
}
